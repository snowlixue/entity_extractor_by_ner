...
def train(configs, data_manager, logger):
    ...
    train_dataset, val_dataset, len_train_dataset, len_val_dataset = data_manager.get_training_set()
    ...
    num_val_iterations = int(math.ceil(1.0 * len_val_dataset / batch_size))
    logger.info(('+' * 20) + 'training starting' + ('+' * 20))
    for i in range(epoch):
        start_time = time.time()
        logger.info('epoch:{}/{}'.format(i + 1, epoch))
        for step, batch in tqdm(train_dataset.shuffle(len_train_dataset).batch(batch_size).enumerate()):
            ...
            if step % configs.print_per_batch == 0 and step != 0:
                batch_pred_sequence, _ = tf.contrib.crf.crf_decode(logits, transition_params, inputs_length)
                measures, _ = metrics(
        ...
        for val_batch in tqdm(val_dataset.batch(batch_size)):
            ...
            batch_pred_sequence_val, _ = tf.contrib.crf.crf_decode(logits_val, transition_params_val, inputs_length_val)
